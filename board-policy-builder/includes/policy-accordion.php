...
class PolicyAccordion {
    ...
    public function policy_accordion_shortcode($atts) {
        ...
        if (empty($all_policies)) {
            ...
            echo '<p><strong>' . esc_html__('No published policies found.', 'board-policy-builder') . '</strong></p>';
            echo '<p>' . esc_html__('Please create some policies first or check if they are published.', 'board-policy-builder') . '</p>';
            echo '</div>';
            return ob_get_clean();
        }
        ...
        if ($atts['show_controls'] === 'true') {
            ...
            echo '<button id="expand-all" class="accordion-toggle-btn">' . esc_html__('Expand All', 'board-policy-builder') . '</button>';
            echo '<button id="collapse-all" class="accordion-toggle-btn">' . esc_html__('Collapse All', 'board-policy-builder') . '</button>';
            echo '</div>';
        }
        ...
        if (!empty($sections) && !is_wp_error($sections)) {
            ...
            echo '<p><strong>' . esc_html__('No policy sections found.', 'board-policy-builder') . '</strong></p>';
            echo '<p>' . sprintf(
                esc_html__('Please create policy sections first: %s', 'board-policy-builder'),
                '<a href="' . esc_url(admin_url('edit-tags.php?taxonomy=policy-section&post_type=policy')) . '">' . 
                esc_html__('Manage Policy Sections', 'board-policy-builder') . '</a>'
            ) . '</p>';
            echo '</div>';
        }
    ...
    public function policy_sections_shortcode($atts) {
        ...
        if (empty($sections) || is_wp_error($sections)) {
            return '<p>' . esc_html__('No policy sections found.', 'board-policy-builder') . '</p>';
        }
        ...
        switch ($atts['format']) {
            ...
            case 'dropdown':
                ...
                echo '<option value="">' . esc_html__('Select a Policy Section...', 'board-policy-builder') . '</option>';
                foreach ($sections as $section) {
                    $count = $atts['show_count'] === 'true' ? ' (' . $section->count . ')' : '';
    ...
    public function policy_list_shortcode($atts) {
        ...
        if (empty($policies)) {
            return '<p>' . esc_html__('No policies found.', 'board-policy-builder') . '</p>';
        }
    ...
    public function policy_debug_shortcode($atts) {
        ...
        echo '<h3>' . esc_html__('Policy Debug Information', 'board-policy-builder') . '</h3>';
        ...
        echo '<p><strong>' . esc_html__('Total Policies:', 'board-policy-builder') . '</strong> ' . count($policies) . '</p>';
        ...
        echo '<p><strong>' . esc_html__('Published Policies:', 'board-policy-builder') . '</strong> ' . count($published_policies) . '</p>';
        ...
        echo '<p><strong>' . esc_html__('Policy Sections:', 'board-policy-builder') . '</strong> ' . count($sections) . '</p>';
        ...
        if (!empty($sections)) {
            ...
            foreach ($sections as $section) {
                ...
                echo '<li>' . esc_html($section->name) . ' (' . count($section_policies) . ' ' . esc_html__('policies', 'board-policy-builder') . ')</li>';
            }
            echo '</ul>';
        }
        ...
        if (!empty($published_policies)) {
            echo '<p><strong>' . esc_html__('Sample Published Policies:', 'board-policy-builder') . '</strong></p>';
            echo '<ul>';
            foreach (array_slice($published_policies, 0, 5) as $policy) {
                $sections = get_the_terms($policy->ID, 'policy-section');
                $section_names = $sections && !is_wp_error($sections) ? wp_list_pluck($sections, 'name') : [__('No section', 'board-policy-builder')];
                echo '<li>' . esc_html($policy->post_title) . ' (' . esc_html__('Section:', 'board-policy-builder') . ' ' . esc_html(implode(', ', $section_names)) . ')</li>';
            }
            echo '</ul>';
        }
        ...
        if ($loop->have_posts()) {
            ...
            <div class="accordion-item">
                ...
                <div class="accordion-content" style="display: none;">
                    <p><em><?php esc_html_e('No published policies in this section yet.', 'board-policy-builder'); ?></em></p>
                </div>
            </div>
            <?php
        }
    ...
    private function render_policies_table($policies, $atts) {
        ...
        echo '<thead><tr>';
        if ($atts['show_code'] === 'true') echo '<th>' . esc_html__('Code', 'board-policy-builder') . '</th>';
        echo '<th>' . esc_html__('Title', 'board-policy-builder') . '</th>';
        if ($atts['show_section'] === 'true') echo '<th>' . esc_html__('Section', 'board-policy-builder') . '</th>';
        echo '<th>' . esc_html__('Adopted', 'board-policy-builder') . '</th>';
        echo '</tr></thead>';
        echo '<tbody>';
    ...
    private function render_policies_cards($policies, $atts) {
        ...
        foreach ($policies as $policy) {
            ...
            if ($atts['show_section'] === 'true' && $sections && !is_wp_error($sections)) {
                ...
                echo '<div class="policy-section">' . esc_html__('Section:', 'board-policy-builder') . ' ' . esc_html(implode(', ', $section_names)) . '</div>';
            }
            if ($adopted) {
                echo '<div class="policy-adopted">' . esc_html__('Adopted:', 'board-policy-builder') . ' ' . esc_html($adopted) . '</div>';
            }
            echo '</div>';
            echo '</div>';
        }
    ...
    public function enqueue_accordion_assets() {
        BPB_Asset_Manager::enqueue_admin_js('policy-accordion-js', 'policy-accordion.js', ['jquery'], BPB_PLUGIN_VERSION, true);
        BPB_Asset_Manager::enqueue_admin_css('policy-accordion-css', 'policy-accordion.css', [], BPB_PLUGIN_VERSION);
    }
    ...
    public function add_admin_menu() {
        add_submenu_page(
            'policy-builder',
            __('Policy Display', 'board-policy-builder'),
            __('Policy Display', 'board-policy-builder'),
            'edit_others_posts',
            'policy-display',
            [$this, 'admin_page']
    ...
    public function admin_page() {
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Policy Display Management', 'board-policy-builder'); ?></h1>
            
            <div class="policy-display-tabs">
                <h2><?php esc_html_e('Available Shortcodes', 'board-policy-builder'); ?></h2>
                
                <div class="shortcode-docs">
                    <h3><?php esc_html_e('Policy Accordion', 'board-policy-builder'); ?></h3>
                    <p><?php esc_html_e('Display policies in collapsible sections:', 'board-policy-builder'); ?></p>
                    <code>[policy_accordion]</code>
                    <p><?php esc_html_e('Options:', 'board-policy-builder'); ?></p>
                    <ul>
                        <li><code>sections</code> - <?php esc_html_e('Comma-separated section slugs (default: all)', 'board-policy-builder'); ?></li>
                        <li><code>show_controls</code> - <?php esc_html_e('Show expand/collapse buttons (default: true)', 'board-policy-builder'); ?></li>
                        <li><code>order</code> - <?php esc_html_e('ASC or DESC (default: ASC)', 'board-policy-builder'); ?></li>
                        <li><code>orderby</code> - <?php esc_html_e('title, date, menu_order (default: title)', 'board-policy-builder'); ?></li>
                    </ul>
                    <p><?php esc_html_e('Example:', 'board-policy-builder'); ?> <code>[policy_accordion sections="administration,board" show_controls="false"]</code></p>
                    
                    <h3><?php esc_html_e('Policy Sections', 'board-policy-builder'); ?></h3>
                    <p><?php esc_html_e('Display list of policy sections:', 'board-policy-builder'); ?></p>
                    <code>[policy_sections]</code>
                    <p><?php esc_html_e('Options:', 'board-policy-builder'); ?></p>
                    <ul>
                        <li><code>format</code> - <?php esc_html_e('list, grid, or dropdown (default: list)', 'board-policy-builder'); ?></li>
                        <li><code>show_count</code> - <?php esc_html_e('Show policy count (default: false)', 'board-policy-builder'); ?></li>
                    </ul>
                    <p><?php esc_html_e('Example:', 'board-policy-builder'); ?> <code>[policy_sections format="grid" show_count="true"]</code></p>
                    
                    <h3><?php esc_html_e('Policy List', 'board-policy-builder'); ?></h3>
                    <p><?php esc_html_e('Display filtered list of policies:', 'board-policy-builder'); ?></p>
                    <code>[policy_list]</code>
                    <p><?php esc_html_e('Options:', 'board-policy-builder'); ?></p>
                    <ul>
                        <li><code>sections</code> - <?php esc_html_e('Comma-separated section slugs', 'board-policy-builder'); ?></li>
                        <li><code>format</code> - <?php esc_html_e('list, table, or cards (default: list)', 'board-policy-builder'); ?></li>
                        <li><code>show_code</code> - <?php esc_html_e('Show policy codes (default: true)', 'board-policy-builder'); ?></li>
                        <li><code>show_section</code> - <?php esc_html_e('Show section names (default: false)', 'board-policy-builder'); ?></li>
                        <li><code>limit</code> - <?php esc_html_e('Number of policies to show (default: -1 for all)', 'board-policy-builder'); ?></li>
                    </ul>
                    <p><?php esc_html_e('Example:', 'board-policy-builder'); ?> <code>[policy_list sections="administration" format="table" show_section="true"]</code></p>
                </div>
                
                <h2><?php esc_html_e('Template Status', 'board-policy-builder'); ?></h2>
                <div class="template-status">
                    ...
                    <p><strong><?php esc_html_e('Policy Accordion Template:', 'board-policy-builder'); ?></strong></p>
                    <ul>
                        <li><?php esc_html_e('Theme template:', 'board-policy-builder'); ?> <?php echo $theme_has_template ? '✅ ' . esc_html__('Found', 'board-policy-builder') : '❌ ' . esc_html__('Not found', 'board-policy-builder'); ?></li>
                        <li><?php esc_html_e('Plugin template:', 'board-policy-builder'); ?> <?php echo $plugin_has_template ? '✅ ' . esc_html__('Available', 'board-policy-builder') : '❌ ' . esc_html__('Not available', 'board-policy-builder'); ?></li>
                    </ul>
                    
                    <?php if (!$theme_has_template && $plugin_has_template): ?>
                        <p class="description"><?php esc_html_e('The plugin will provide the template since your theme doesn\'t have one.', 'board-policy-builder'); ?></p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
