<?php
/**
 * Policy Search & Highlighting Integration with Auto-Detection - FIXED VERSION
...
class PolicySearch {
    ...
    public function init() {
        // ✅ FIXED: Start session early, before any output
    ...
    /**
     * ✅ FIXED: Start session early and safely
     */
    ...
    /**
     * ✅ FIXED: Capture search context safely
     */
    public function capture_search_context() {
        if ($this->is_search_context()) {
            $search_term = $this->get_current_search_term();
            if ($search_term) {
                // ✅ FIXED: Use WordPress transients instead of sessions
                // This avoids session issues entirely
    ...
    /**
     * ✅ NEW: Get unique user key for transients (replaces session)
     */
    ...
    /**
     * ✅ FIXED: Enhanced search term detection using transients instead of sessions
     */
    private function get_search_term() {
        ...
        // ✅ FIXED: Use transients instead of sessions
        $user_key = $this->get_user_key();
    ...
    private function get_search_results($atts) {
        ...
        if ($search_query->have_posts()) {
            echo '<h3 class="search-results-title">' . sprintf(esc_html__('Search Results for "%s"', 'board-policy-builder'), esc_html(get_search_query())) . '</h3>';
            echo '<div class="search-results-count">' . sprintf(esc_html__('Found %d result(s)', 'board-policy-builder'), $search_query->found_posts) . '</div>';
    ...
    */
    ...
    public function modify_search_query($query) {
        ...
        if ($query->get('post_type') === 'policy') {
            ...
            if (!class_exists('SearchWP')) {
                ...
                add_filter('posts_search', [$this, 'custom_policy_search'], 10, 2);
    ...
    /**
     * Custom search function for better policy code matching
     */
    public function custom_policy_search($search, $wp_query) {
        global $wpdb;
        ...
        $search = " AND (
            ...
            OR EXISTS (
                ...
                AND (
                    (pm.meta_key = 'code-policy' AND pm.meta_value LIKE '%{$search_term}%')
                    OR (pm.meta_key = 'title-policy' AND pm.meta_value LIKE '%{$search_term}%') 
                    OR (pm.meta_key = 'policy' AND pm.meta_value LIKE '%{$search_term}%')
                    OR (pm.meta_key = '_code-policy' AND pm.meta_value LIKE '%{$search_term}%')
                    OR (pm.meta_key = '_title-policy' AND pm.meta_value LIKE '%{$search_term}%')
                    OR (pm.meta_key = '_policy' AND pm.meta_value LIKE '%{$search_term}%')
                )
            )
        )";
