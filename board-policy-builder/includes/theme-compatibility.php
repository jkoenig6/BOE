...
class PolicyThemeCompatibility {
    ...
    public function enqueue_policy_frontend_assets() {
        ...
        if (is_singular('policy') || is_post_type_archive('policy') || is_tax('policy-section')) {
            ...
            if (file_exists($theme_policy_css)) {
                ...
                BPB_Asset_Manager::enqueue_frontend_css('policy-frontend-plugin', 'policy-frontend.css', [], BPB_PLUGIN_VERSION);
            }
        }
    ...
    public function fix_admin_asset_paths($hook) {
        ...
        BPB_Asset_Manager::enqueue_admin_css('policy-admin-plugin', 'policy-admin.css', [], BPB_PLUGIN_VERSION);
    }
    ...
    public static function get_asset_url($asset_path, $asset_type = 'css') {
        ...
        return BPB_Asset_Manager::get_asset_url($asset_path, $asset_type, $context);
    }
    ...
    public static function migrate_from_theme() {
        ...
        $theme_files_to_check = [
            'generate-pdf.php' => __('PDF generator', 'board-policy-builder'),
            'page-policy-accordion-basic.php' => __('Policy accordion template', 'board-policy-builder'),
            'css/policy-accordion.css' => __('Policy accordion styles', 'board-policy-builder'),
            'js/policy-accordion.js' => __('Policy accordion JavaScript', 'board-policy-builder'),
            'js/highlight-links.js' => __('Search highlighting script', 'board-policy-builder'),
            'js/olstart-plugin.js' => __('TinyMCE plugin', 'board-policy-builder'),
            'single-policy.php' => __('Single policy template', 'board-policy-builder')
        ];
        ...
        foreach ($theme_files_to_check as $file => $description) {
            ...
            if (file_exists($file_path)) {
                $migration_log[] = [
                    ...
                    'action_needed' => __('Can be removed from theme - plugin provides this functionality', 'board-policy-builder')
                ];
            }
        }
        ...
        $files_to_scan = [
            BPB_PLUGIN_PATH . 'board-policy-builder.php',
            BPB_PLUGIN_PATH . 'includes/policy-builder.php',
            BPB_PLUGIN_PATH . 'includes/policy-templates.php'
        ];
            ...
            if (file_exists($file_path)) {
                ...
                if (strpos($content, 'astra-child') !== false) {
                    $migration_log[] = [
                        ...
                        'description' => __('Contains hardcoded theme reference', 'board-policy-builder'),
                        'status' => 'needs_update',
                        'action_needed' => __('Update to use dynamic theme detection', 'board-policy-builder')
                    ];
                }
            }
        }
    ...
    public static function display_migration_status() {
        ...
        if (empty($migration_log)) {
            echo '<div class="notice notice-success"><p>✅ ' . esc_html__('No theme migration issues detected.', 'board-policy-builder') . '</p></div>';
            return;
        }
        
        echo '<div class="migration-status-panel">';
        echo '<h3>' . esc_html__('Theme Migration Status', 'board-policy-builder') . '</h3>';
        echo '<table class="widefat">';
        echo '<thead><tr>';
        echo '<th>' . esc_html__('File', 'board-policy-builder') . '</th>';
        echo '<th>' . esc_html__('Description', 'board-policy-builder') . '</th>';
        echo '<th>' . esc_html__('Status', 'board-policy-builder') . '</th>';
        echo '<th>' . esc_html__('Action Needed', 'board-policy-builder') . '</th>';
        echo '</tr></thead>';
        echo '<tbody>';
