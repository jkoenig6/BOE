...
class PolicyPDFGenerator {
    ...
    public function enqueue_pdf_assets() {
        if (is_singular('policy')) {
            BOE_Asset_Manager::enqueue_frontend_css('policy-pdf-styles', 'policy-pdf.css', [], BOE_PLUGIN_VERSION);
        }
    }
    ...
    private function get_pdf_icon_url() {
        ...
        return BOE_Asset_Manager::get_asset_url('icons/pdf-icon.png', 'image');
    }
    ...
    private function get_logo_url() {
        ...
        return BOE_Asset_Manager::get_asset_url('icons/default-logo.png', 'image');
    }
    ...
    public function pdf_debug_shortcode($atts) {
        ...
        echo '<h3>' . esc_html__('PDF Debug Information', 'boe-meeting-manager') . '</h3>';
        
        // Check DOMPDF
        $dompdf_available = $this->load_dompdf();
        echo '<p><strong>' . esc_html__('DOMPDF Available:', 'boe-meeting-manager') . '</strong> ' . ($dompdf_available ? '✅ ' . esc_html__('Yes', 'boe-meeting-manager') : '❌ ' . esc_html__('No', 'boe-meeting-manager')) . '</p>';
        
        if (!$dompdf_available) {
            $check_paths = [
                BOE_PLUGIN_PATH . 'includes/dompdf/autoload.inc.php',
                BOE_PLUGIN_PATH . 'vendor/dompdf/autoload.inc.php',
                BOE_PLUGIN_PATH . 'includes/dompdf/autoload.inc.php'
            ];
            
            echo '<p><strong>' . esc_html__('Checked paths:', 'boe-meeting-manager') . '</strong></p><ul>';
        ...
        if (is_singular('policy')) {
            ...
            echo '<p><strong>' . esc_html__('Current Policy:', 'boe-meeting-manager') . '</strong> ' . esc_html($post->post_title) . '</p>';
            echo '<p><strong>' . esc_html__('Policy Code:', 'boe-meeting-manager') . '</strong> ' . ($code ? esc_html($code) : '❌ ' . esc_html__('Missing', 'boe-meeting-manager')) . '</p>';
            echo '<p><strong>' . esc_html__('Policy Title Field:', 'boe-meeting-manager') . '</strong> ' . ($title_policy ? esc_html($title_policy) : '❌ ' . esc_html__('Missing', 'boe-meeting-manager')) . '</p>';
            
            $should_show = $this->should_show_pdf_link($post->ID);
            echo '<p><strong>' . esc_html__('Should Show PDF Link:', 'boe-meeting-manager') . '</strong> ' . ($should_show ? '✅ ' . esc_html__('Yes', 'boe-meeting-manager') : '❌ ' . esc_html__('No', 'boe-meeting-manager')) . '</p>';
            ...
            if ($should_show) {
                ...
                echo '<p><strong>' . esc_html__('PDF URL:', 'boe-meeting-manager') . '</strong> <a href="' . esc_url($pdf_url) . '" target="_blank">' . esc_html__('Test PDF Generation', 'boe-meeting-manager') . '</a></p>';
            }
        }
